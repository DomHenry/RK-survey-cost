...
source("functions/instruction_text.R")
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    width = 150,
    sidebarMenu(
      menuItem("Instructions", tabName = "instruct", icon = icon("info-circle")), # Pick better logo
      menuItem("Survey design", tabName = "complot", icon = icon("chart-area")),
      menuItem("Downloads", tabName = "download", icon = icon("download")),
      menuItem("Citations", tabName = "citation", icon = icon("file-alt"))
    )
  ),
  ...
  dashboardBody(
    tabItems(
      tabItem(
        ...
        fluidRow( 
          box(
            title = "Instructions", width = 8,
            status = "primary", solidHeader = TRUE,
            in_text # Explanatory intro text sourced from function
          )
        )
      ),
